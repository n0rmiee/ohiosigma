...
local function searchClosure(script, name, upvalueIndex, constants)
    for _i, v in pairs(getGc()) do
        if type(v) == "function" then  -- Ensure v is a function
            local parentScript = rawget(getfenv(v), "script")
            if isLClosure(v) and not isXClosure(v) then
                if (script == nil and parentScript.Parent == nil) or script == parentScript then
